import { Disposable, EventEmitter, Event, commands } from 'vscode';

export interface DuckMessage {
  type: string,
  domElementId: string
}

export default class DuckModeQuack
{
  public static commandName:string = "duck-mode.quack";

  private _onDidActivateEmitter: EventEmitter<DuckMessage> = new EventEmitter<DuckMessage>
  public onDidActivate: Event<DuckMessage>;

  constructor() {
    this.onDidActivate = this._onDidActivateEmitter.event
  }

  register() : Disposable {

    /**
     * This registers commands with VS Code that will be called when certain
     * actions are taken in the IDE itself, such as button or menu item clicks
     *
     * This then executes functionality in our extension, when we have our own event
     * emitters to communicate within the extension
     */
    return commands.registerCommand(DuckModeQuack.commandName, () => {

      const audio: DuckMessage = {
        type: 'audio',
        domElementId: 'audio-quack'
      }
      this._onDidActivateEmitter.fire(audio);
    })
  }
}
